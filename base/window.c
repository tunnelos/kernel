...
int __window_offsetx = 325;
int __window_offsety = 0;
...
void __window_save_context(char *old_context, window_t *new_context) {
    int x1 = (new_context->wx + __window_offsetx) * 8; int x2 = x1 + (new_context->sx + __window_offsetx) * 8;
    int y1 = (new_context->wy + __window_offsety) * 16; int y2 = y1 + (new_context->sy + __window_offsety) * 16;
...
void __window_read_old_ctx(char *old_context, window_t *crt_context) {
    int x1 = (crt_context->wx + __window_offsetx) * 8; int x2 = x1 + (crt_context->sx + __window_offsetx) * 8;
    int y1 = (crt_context->wy + __window_offsety) * 16; int y2 = y1 + (crt_context->sy + __window_offsety) * 16;
    int ocp = 0; int s = tunnelos_sysinfo.bootboot.fb_scanline;
    uint8_t *fbp = (uint8_t *)&fb;
