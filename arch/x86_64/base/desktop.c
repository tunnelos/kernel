...
void __desktop_init() {
    ...
    //draw bg
    __gui_drawRectangle((vector2d_t){0, 0}, (vector2d_t){80, 30}, currentTheme.bgcolor, NULL);

    //draw task bar
    for(int i = 0; i < 80; i++) putc('\x08', __color_to_int(currentTheme.taskbarcolor), i, 0);
    __gui_drawRectangle((vector2d_t){0, 0}, (vector2d_t){80, 1}, currentTheme.taskbarcolor, NULL);

    //draw start button
    //putc('+', __color_to_int(COLOR_WHITE), 0, 0);
    __gui_drawText((vector2d_t){0, 0}, (vector2d_t){1, 1}, COLOR_WHITE, "+", NULL);
...
void __desktop_render_tasks(){
    ...
    for(int i = 0; i < 512; i++){
        if(tasks[i].used) {
            ...
            __gui_drawText((vector2d_t){x, 0}, (vector2d_t){strlen(tasks[i].title), 1}, COLOR_WHITE, tasks[i].title, NULL);
        }
    }
}
...
void __desktop_render_categories(){
    //draw categories
    __gui_drawRectangle((vector2d_t){0, 1}, (vector2d_t){10, 29}, COLOR_GRAY, NULL);

    for(uint16_t i = 0; i < categoriesCount; i++){
        if(currentCategory == i) __gui_drawRectangle((vector2d_t){i, 1 + currentCategory}, (vector2d_t){10, 1}, currentTheme.bgcolor, NULL);
        __gui_drawText((vector2d_t){1, 1 + currentCategory}, (vector2d_t){9, 1}, COLOR_WHITE, categories[currentCategory].title, NULL);
    }
}